...
async function api(path, { method = "GET", body } = {}) {
  const r = await fetch(alteredUrl(path), {
    method,
  ...
  if (r.status === 401) {
    window.location.href = alteredUrl(p?.loginUrl || "/auth/ubisoft/login?return_to=%2Fadmin%2Fmonitoring%2F");
...
async function loadAuth() {
  ...
  if (!a?.authenticated) {
    window.location.href = alteredUrl(a?.loginUrl || "/auth/ubisoft/login?return_to=%2Fadmin%2Fmonitoring%2F");
    return false;
  }
  return true;
}
