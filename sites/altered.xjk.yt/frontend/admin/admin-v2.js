...
const CAMPAIGN_CATALOG_PAGE_SIZE = 200;
const CAMPAIGN_LABEL_COLLATOR = new Intl.Collator(undefined, { numeric: true, sensitivity: "base" });
const DEFAULT_SIMILARITY_WEIGHT_PROFILE = {
...
const SIMILARITY_WEIGHT_SECTIONS = [
  {
    key: "final",
    label: "Final Blend",
    fields: [
...
const NAMING_SIMILARITY_FALLBACK_CAMPAIGNS = {
  "official-discovery": [
    "Snow Discovery",
    "Rally Discovery",
    "Desert Discovery",
...
const NAMING_SIMILARITY_SEASON_OPTIONS = [
  ["", "All Seasons"],
  ["Winter", "Winter"],
  ["Spring", "Spring"],
  ["Summer", "Summer"],
...
const NAMING_SIMILARITY_MONTH_OPTIONS = [
  ["", "All Months"],
  ["01", "January"],
  ["02", "February"],
  ["03", "March"],
...
const NAMING_SIMILARITY_TOTD_START_UTC = Date.UTC(2020, 6, 1);
const NAMING_SIMILARITY_WEEKLY_EPOCH_UTC = Date.UTC(2024, 11, 15);
const NAMING_SIMILARITY_WEEKLY_GRANDS_MIN_WEEK = 59;
...
function resolveWorkspaceHref(path, service = "") {
  ...
  if (service === "aggregator") {
    ...
    if (host === "localhost" || host === "127.0.0.1") {
      return `${window.location.origin}/aggregator${safePath}`;
    }
    if (host.endsWith(".localhost")) {
      return `${window.location.protocol}//aggregator.localhost${port}${safePath}`;
    }
    return `https://aggregator.xjk.yt${safePath}`;
  }
...
function renderSession() {
  ...
  if (a.authenticated) {
    ...
    const url = normalizeLoginUrl(a.loginUrl || "/admin/login/");
    el.sidebarSession.innerHTML = `
...
async function loadCampaignCatalog() {
  ...
  campaignCatalogPromise = (async () => {
    try {
      ...
      let page = 1;
      while (true) {
        const data = await api(`/api/v1/admin/maps/workspace?view=campaigns&page=${page}&pageSize=${CAMPAIGN_CATALOG_PAGE_SIZE}`);
...
function logJobsConsole(message, details = null, level = "log") {
...
function logJobsOverviewConsole(payload, { source = "jobs-refresh", force = false } = {}) {
  ...
  if (!previousEntries || force || changes.length > 0) {
    console.log("[altered-admin/jobs] raw jobs payload", payload);
  }
...
function buildLiveMonitorConsoleSnapshot(payload = {}) {
  ...
  return {
    ...
    trackerChunkSize:
      Number.isFinite(Number(counters?.trackerChunkSize)) ? Number(counters.trackerChunkSize) : null,
    trackerChunksSynced:
      Number.isFinite(Number(counters?.trackerChunksSynced)) ? Number(counters.trackerChunksSynced) : null,
    trackerChunksTotal:
      Number.isFinite(Number(counters?.trackerChunksTotal)) ? Number(counters.trackerChunksTotal) : null,
    trackerMapsSynced:
      Number.isFinite(Number(counters?.trackerMapsSynced)) ? Number(counters.trackerMapsSynced) : null,
    trackerMapsToSync:
      Number.isFinite(Number(counters?.trackerMapsToSync)) ? Number(counters.trackerMapsToSync) : null,
    currentMaps,
  };
}
...
function logLiveMonitorConsole(payload, { source = "jobs-live-status", force = false } = {}) {
  ...
  console.log("[altered-admin/jobs] raw live status payload", payload);
...
async function loadLiveMonitorStatusForJobs({ source = "jobs-live-status", forceConsole = false } = {}) {
...
function parseSeasonLabel(label = "") {
  const match = String(label || "").trim().match(/^(training|winter|spring|summer|fall|autumn)(?:\s+(\d{4}))?$/i);
...
function getCampaignOptionsForSource(sourceKey = "") {
  const normalizedSourceKey = String(sourceKey || "").trim().toLowerCase();
...
function getNamingSimilarityYearOptions() {
  const startYear = 2020;
  const endYear = Math.max(startYear, new Date().getUTCFullYear());
  const years = [];
...
function getWeeklyEpochDateUtc() {
  return new Date(NAMING_SIMILARITY_WEEKLY_EPOCH_UTC);
}
...
function parseWeeklyCatalogEntry(campaign = {}, sourceKey = "") {
  const normalizedSourceKey = String(sourceKey || "").trim().toLowerCase();
  const name = String(campaign?.name || "").trim();
  const pattern =
    normalizedSourceKey === "weekly-grands"
      ? /\bweek\s*grand(?:s)?\s*0*(?<week>\d{1,3})\b/i
      : /\bweek\s*0*(?<week>\d{1,3})\b/i;
  let match = name.match(pattern);
  if (!match?.groups?.week && normalizedSourceKey === "weekly-shorts") {
    match = name.match(/(?:^|\D)0*(?<week>\d{1,3})(?!.*\d)/);
  }
  const week = Number(match?.groups?.week || 0) || 0;
  const timestampMs =
    Number(campaign?.sort_timestamp_ms || campaign?.sortTimestampMs || 0) ||
    Date.parse(String(campaign?.added_at || "")) ||
    0;
  const year = timestampMs > 0 ? new Date(timestampMs).getUTCFullYear() : 0;
  if (!week || !year) return null;
  return { year, week };
}

function getSeasonalYearOptions() {
...
function getTotdMonthOptions(yearValue = "") {
...
function getTotdDayOptions(yearValue = "", monthValue = "") {
  const year = Number(yearValue || 0) || 0;
  ...
  const startDay = year === startDate.getUTCFullYear() && month === startDate.getUTCMonth() + 1
    ? startDate.getUTCDate()
    : 1;
  const endDay = year === today.getUTCFullYear() && month === today.getUTCMonth() + 1
    ? today.getUTCDate()
...
function getWeeklyYearOptions(sourceKey = "") {
...
function getWeeklyWeekOptions(sourceKey = "", yearValue = "") {
  const year = Number(yearValue || 0) || 0;
...
function isWeeklySourceKey(sourceKey = "") {
  const normalizedSourceKey = String(sourceKey || "").trim().toLowerCase();
...
function getNamingSimilarityScopeError() {
...
function resetNamingSimilarityScopedSelections() {
...
function deriveNamingSimilarityCampaignName() {
  const sourceKey = String(state.namingSimilaritySourceKey || "").trim().toLowerCase();
  ...
  if (sourceKey === "weekly-shorts" || sourceKey === "weekly-grands") {
    ...
    if (week) {
      state.namingSimilarityCampaignName =
        sourceKey === "weekly-grands" ? `Week Grand ${week}` : `Week ${week}`;
      return state.namingSimilarityCampaignName;
    }
    state.namingSimilarityCampaignName = year || "";
    return state.namingSimilarityCampaignName;
...
function rerenderNamingSimilarityControlSurfaces() {
  document.querySelectorAll("[data-naming-similarity-controls]").forEach((node) => {
    ...
    node.outerHTML = renderSimilarityBackfillControls({
      variant,
      buttonLabel,
      buttonClass,
    });
...
function syncNamingSimilarityCampaignSelects() {
  const options = getCampaignOptionsForSource(state.namingSimilaritySourceKey || "");
  const normalizedSourceKey = String(state.namingSimilaritySourceKey || "").trim().toLowerCase();
...
function renderSimilarityBackfillControls({
  ...
  if (variant === "workspace") {
    ...
    if (normalizedSourceKey === "official-seasonal-v2") {
      ...
      const renderedWeeklyWeekOptions =
        optionListHasValue(weeklyWeekOptions, state.namingSimilarityWeek || "")
          ? weeklyWeekOptions
          : [[String(state.namingSimilarityWeek || ""), `Invalid week ${state.namingSimilarityWeek || ""}`], ...weeklyWeekOptions];
      sourceSpecificFieldsMarkup = `
        <label class="field">
          <span>Year</span>
          <select data-naming-similarity-year>
...
function namingPresetKey(filters = {}) {
  const reviewState = String(filters.reviewState || "").trim().toLowerCase();
  const automationState = String(filters.automationState || "").trim().toLowerCase();
  const requiresRegex = String(filters.requiresRegex || "").trim().toLowerCase();
...
function renderNamingMetricCard({
  label,
  value,
  note = "",
  accent = "info",
        ...
        <form data-form-kind="maps-filters" class="naming-filter-panel">
          ...
          <div class="naming-filter-grid">
            ...
            <label class="field">
              ...
              <select name="automationState">${selOpts([["","All"],["matched","Matched"],["unmatched","Unmatched"]], filters.automationState)}</select>
            </label>
            <label class="field">
              <span>Review</span>
              <select name="reviewState">${selOpts([["","All"],["pending","Pending"],["approved","Approved"],["ignored","Ignored"]], filters.reviewState)}</select>
            </label>
            <label class="field">
              <span>Requires Regex</span>
              <select name="requiresRegex">${selOpts([["","All"],["true","Yes"],["false","No"]], filters.requiresRegex)}</select>
            </label>
            <label class="field">
              <span>Rows</span>
              <select name="pageSize">${selOpts([["10","10 / page"],["25","25 / page"],["50","50 / page"],["100","100 / page"]], String(state.maps.pageSize.naming || payload.pageSize || 10))}</select>
            </label>
          </div>
...
function endpointLiveHref(endpoint) {
  ...
  if (endpoint?.key === "public-map-detail" || endpoint?.key === "legacy-map-info") return resolveWorkspaceHref("/api/");
  return "";
}
...
function buildAdminSimilarityWeightProfile(profile = null, baseProfile = DEFAULT_SIMILARITY_WEIGHT_PROFILE) {
  const base = baseProfile && typeof baseProfile === "object"
  ...
  return {
    final: {
      ...
      weightedPlacement: clampSimilarityWeightPercent(
      ...
      name: clampSimilarityWeightPercent(
        final.name ?? raw.nameSupport,
        base.final.name ?? base.nameSupport ?? 0
      ),
      regex: regexOverwriteWeights
        ? clampSimilarityWeightPercent(final.regex, base.final.regex ?? 0)
        : 0,
    },
    weightedPlacement: {
      absolute: clampSimilarityWeightPercent(
        weightedPlacement.absolute,
        base.weightedPlacement.absolute
      ),
      relative: clampSimilarityWeightPercent(
    ...
    relationalFallback: {
      ...
      model: clampSimilarityWeightPercent(
...
function formatSimilarityWeightSummary(profile = null) {
...
function renderSimilarityWeightInputs(
  profile = null,
  ...
  const sectionsMarkup = SIMILARITY_WEIGHT_SECTIONS.map((section) => `
    <section
...
function syncSimilarityWeightRegexVisibility(scope = document) {
  const root =
    scope instanceof Document || scope instanceof HTMLElement ? scope : document;
...
function resolveAlterationRuleInputValue(alterationSlug = "", alterations = []) {
  const raw = String(alterationSlug || "").trim();
  if (!raw) return "";
  const match = (Array.isArray(alterations) ? alterations : []).find(
    (item) => String(Array.isArray(item) ? item[0] : item?.slug || "").trim().toLowerCase() === raw.toLowerCase()
  );
...
function renderAlterationRuleInput(alterations = [], value = "") {
  ...
  const suggestions = [...new Map(
    (Array.isArray(alterations) ? alterations : [])
      ...
      .filter(Boolean)
  ).values()].sort((a, b) => CAMPAIGN_LABEL_COLLATOR.compare(a.label, b.label));
  return `
...
function slugifyAdminWeightValue(value = "") {
  return String(value || "")
    .trim()
    .normalize("NFKD")
    .replace(/[^\x00-\x7F]/g, "")
...
function resolveAlterationRuleInputSlug(input) {
  return resolveAlterationRuleInputSelection(input)?.slug || "";
}
...
function getAlterationRegexEntries(alterationRegexLibrary = {}, alterationSlug = "") {
  ...
  const entries = Array.isArray(rawEntries)
    ? rawEntries
    : Array.isArray(rawEntries?.entries)
      ? rawEntries.entries
      : [];
...
function renderSelectedAlterationRegexHints({
...
function syncSelectedAlterationRegexHints(scope = document) {
  const root =
    scope instanceof Document || scope instanceof HTMLElement ? scope : document;
  root.querySelectorAll("[data-selected-alteration-regex-panel]").forEach((panel) => {
    ...
    panel.innerHTML = renderSelectedAlterationRegexHints({
...
function renderSimilarityScopeGrid({ sourceOptions, seasonOptions, yearOptions, environmentOptions, alterationOptions, ruleDraft }) {
  return `
...
function hideAlterationSearchLists(exceptShell = null) {
...
function updateAlterationSearchSuggestions(input, { showAllOnEmpty = false } = {}) {
  if (!(input instanceof HTMLInputElement)) return;
  const shell = input.closest("[data-alteration-search]");
  ...
  options.forEach((option) => {
    const searchText = String(option.getAttribute("data-search-text") || "").trim().toLowerCase();
...
function parseSimilarityWeightProfileFromFormData(fd, baseProfile = DEFAULT_SIMILARITY_WEIGHT_PROFILE) {
...
function renderSimilarityWeightScopeCard({
  scope = "map",
  title = "",
  description = "",
  mapUid = "",
  ...
  const toneClass =
    hasOverride || activeScope === scopeKey ? "tone-info" : "tone-muted";
  if (disabled) {
    return `
...
function resetSimilarityWeightRuleDraft() {
...
function resetSimilarityWeightCampaignDraft(campaignId = "") {
...
function getSimilarityWeightSourceOptions() {
  ...
  (Array.isArray(state.campaignCatalog) ? state.campaignCatalog : []).forEach((campaign) => {
    ...
    seen.add(sourceKey);
    const label =
      NAMING_SIMILARITY_SOURCE_OPTIONS.find(([value]) => value === sourceKey)?.[1] ||
      sourceKey;
  ...
  const options = rows
    ...
    .map((campaign) => {
      ...
      const sourceLabel =
        NAMING_SIMILARITY_SOURCE_OPTIONS.find(
          ([value]) =>
            value === String(campaign?.source_classification || campaign?.source_key || "").trim().toLowerCase()
        )?.[1] || campaign?.source_classification || campaign?.source_key || "";
      const seasonLabel =
        [campaign?.season, campaign?.season_year].filter(Boolean).join(" ");
      const alterationLabel = Array.isArray(campaign?.alterations) && campaign.alterations.length
        ? campaign.alterations.map((item) => item?.name).filter(Boolean).join(" + ")
        : "";
...
function buildSimilarityWeightAlterationOptions(alterations = []) {
  ...
  return [
    ["", "All Alterations"],
    ...merged.sort((a, b) => CAMPAIGN_LABEL_COLLATOR.compare(a[1], b[1])),
  ];
}
...
function findCampaignCatalogRowById(campaignId) {
  ...
  return (Array.isArray(state.campaignCatalog) ? state.campaignCatalog : []).find(
...
function formatSimilarityWeightRuleScope(rule = {}, alterations = []) {
  ...
  if (rule?.alterationSlug) {
    const alteration = (Array.isArray(alterations) ? alterations : []).find(
      (item) => String(item?.slug || "").trim().toLowerCase() === String(rule.alterationSlug || "").trim().toLowerCase()
    );
...
function renderSimilarityWeightsWorkspace(payload = {}) {
  ...
  const campaignDraftProfile = buildAdminSimilarityWeightProfile(campaignDraft.profile || defaultProfile, defaultProfile);
  ...
  const selectedRuleAlteration = (() => {
    ...
    const exact = (Array.isArray(alterationOptions) ? alterationOptions : []).find(
      (item) => String(Array.isArray(item) ? item[1] : item?.name || "").trim().toLowerCase() === resolvedValue.toLowerCase()
    );
  ...
  return `
    <section class="naming-workspace-shell">
      ...
      ${tableCard("Scoped Rules", `${fmtNum(scopedRules.length)} saved`, `
        <table class="data-table">
          <thead><tr><th>Scope</th><th>Weights</th><th>Updated</th><th></th></tr></thead>
          <tbody>
            ${scopedRules.map((rule) => `<tr>
            ...
            </tr>`).join("") || `<tr><td colspan="4"><p class="inline-empty">No scoped similarity weight rules yet.</p></td></tr>`}
          </tbody>
        </table>
      `)}

      ${tableCard("Campaign Overrides", `${fmtNum(campaignOverrides.length)} saved`, `
        <table class="data-table">
          ...
          <tbody>
            ${campaignOverrides.map((override) => {
              ...
              const meta = [campaign?.season_label || "", campaign?.source_classification || campaign?.source_key || ""].filter(Boolean).join(" · ");
              return `<tr>
            ...
            }).join("") || `<tr><td colspan="4"><p class="inline-empty">No campaign overrides saved yet.</p></td></tr>`}
          </tbody>
        </table>
      `)}
    </section>
...
function renderNamingDetailDrawer(payload) {
  ...
  const similarityWarnings = Array.isArray(similarityDetails?.diagnosticWarnings)
    ...
    : [];
  const defaultWeightProfile = buildAdminSimilarityWeightProfile(
    similarityWeights.defaults || DEFAULT_SIMILARITY_WEIGHT_PROFILE
  );
  const campaignWeightProfile = buildAdminSimilarityWeightProfile(
    similarityWeights?.campaignOverride?.weights || null,
    defaultWeightProfile
  );
  const effectiveWeightProfile = buildAdminSimilarityWeightProfile(
    similarityWeights.effective || similarityDetails?.weightProfile?.raw || null,
    defaultWeightProfile
  );
  const mapWeightProfile = buildAdminSimilarityWeightProfile(
    similarityWeights?.mapOverride?.weights || null,
  ...
  const activeWeightScope = String(
    ...
    .trim()
    .toLowerCase();
  const activeWeightSummary = formatSimilarityWeightSummary(effectiveWeightProfile);
  ...
  kvs.push(kv("Similarity Weight Profile", `${activeWeightSummary} (${activeWeightScope || "default"})`));
  ...
  el.drawerBody.innerHTML = `
    ...
    <section class="drawer-tabpanel" data-drawer-tab-panel="similarity" hidden>
      <div class="drawer-section">
        ...
        ${similarityWarningMarkup}
        <div class="similarity-weight-grid" style="margin-top:.6rem;">
...
function setSimilarityWeightEditorTab(editor, tabKey = "final") {
  if (!(editor instanceof HTMLElement)) return;
...
async function onClick(e) {
  ...
  if (!(t instanceof HTMLElement) || !t.closest("[data-alteration-search]")) {
  ...
  const similarityWeightTab = t.closest("[data-similarity-weight-tab]");
  if (similarityWeightTab) {
    setSimilarityWeightEditorTab(
      similarityWeightTab.closest("[data-similarity-weight-editor]"),
      similarityWeightTab.getAttribute("data-similarity-weight-tab") || "final"
    );
    return;
  }
  ...
  const alterationOption = t.closest("[data-alteration-option]");
  if (alterationOption) {
    const shell = alterationOption.closest("[data-alteration-search]");
    const input = shell?.querySelector("[data-alteration-search-input]");
    if (input instanceof HTMLInputElement) {
      input.value = String(alterationOption.getAttribute("data-alteration-option") || "").trim();
      const list = shell?.querySelector("[data-alteration-search-list]");
  ...
  if (t.closest("[data-reset-maps]")) {
    state.maps.filters[state.maps.view] = {
      inventory: { q: "", campaign: "", tracked: "", status: "", staleState: "" },
      campaigns: {},
      naming: { q: "", automationState: "", reviewState: "pending", requiresRegex: "" },
      weights: {},
  ...
  const resetWeightRuleForm = t.closest("[data-reset-similarity-weight-rule-form]");
  if (resetWeightRuleForm) {
    resetSimilarityWeightRuleDraft();
    if (state.maps.view === "weights" && state.maps.data) renderMaps();
    return;
  }

  const resetWeightCampaignForm = t.closest("[data-reset-similarity-weight-campaign-form]");
  if (resetWeightCampaignForm) {
    resetSimilarityWeightCampaignDraft();
    if (state.maps.view === "weights" && state.maps.data) renderMaps();
    return;
  }

  const editWeightRule = t.closest("[data-similarity-weight-rule-edit]");
  if (editWeightRule) {
    const ruleId = Number(editWeightRule.getAttribute("data-similarity-weight-rule-edit") || 0) || 0;
    ...
    state.similarityWeightsWorkspace.ruleDraft = {
      ...
      profile: buildAdminSimilarityWeightProfile(rule.weights || DEFAULT_SIMILARITY_WEIGHT_PROFILE),
    };
  ...
  const deleteWeightRule = t.closest("[data-similarity-weight-rule-delete]");
  if (deleteWeightRule) {
    const ruleId = Number(deleteWeightRule.getAttribute("data-similarity-weight-rule-delete") || 0) || 0;
    if (!ruleId) return;
    await guarded(`delete-similarity-weight-rule:${ruleId}`, async () => {
      const payload = await post(`/api/v1/admin/similarity-weight-rules/${encodeURIComponent(String(ruleId))}/delete`, {});
  ...
  const editWeightCampaign = t.closest("[data-similarity-weight-campaign-edit]");
  if (editWeightCampaign) {
    const campaignId = Number(editWeightCampaign.getAttribute("data-similarity-weight-campaign-edit") || 0) || 0;
    const override = (Array.isArray(state.maps.data?.campaignOverrides) ? state.maps.data.campaignOverrides : []).find(
      (entry) => Number(entry?.campaignId || 0) === campaignId
    );
    if (!override) return;
    state.maps.view = "weights";
    state.similarityWeightsWorkspace.campaignDraft = {
      campaignId: String(campaignId),
      profile: buildAdminSimilarityWeightProfile(override.weights || DEFAULT_SIMILARITY_WEIGHT_PROFILE),
    };
    setHash("maps", { view: "weights" });
    renderMaps();
    return;
  ...
  const deleteWeightCampaign = t.closest("[data-similarity-weight-campaign-delete]");
  if (deleteWeightCampaign) {
    const campaignId = Number(deleteWeightCampaign.getAttribute("data-similarity-weight-campaign-delete") || 0) || 0;
    if (!campaignId) return;
    await guarded(`delete-similarity-weight-campaign:${campaignId}`, async () => {
      const payload = await post(`/api/v1/admin/similarity-weight-campaign-overrides/${encodeURIComponent(String(campaignId))}/delete`, {});
      if (payload?.workspace) {
        state.maps.data = payload.workspace;
        state.maps.lastRequestKey = "weights";
      } else {
  ...
  const openSimilarityWeights = t.closest("[data-open-similarity-weights]");
  if (openSimilarityWeights) {
    const campaignId = String(openSimilarityWeights.getAttribute("data-open-similarity-weights") || "").trim();
    state.maps.view = "weights";
  ...
  const namingPreset = t.closest("[data-naming-preset]");
  if (namingPreset) {
    const preset = String(namingPreset.getAttribute("data-naming-preset") || "").trim().toLowerCase();
    const current = state.maps.filters.naming || {};
    const next = {
      q: String(current.q || ""),
      automationState: "",
      reviewState: "",
    ...
    if (preset === "pending") {
      ...
      next.reviewState = "pending";
      next.automationState = "unmatched";
    } else if (preset === "matched-pending") {
      next.reviewState = "pending";
      next.automationState = "matched";
    } else if (preset === "regex") {
      next.reviewState = "pending";
      next.requiresRegex = "true";
    ...
    state.maps.filters.naming = next;
    ...
    await guarded("naming-similarity", async () => {
      ...
      try {
        ...
        const emptySelection = Boolean(
          kickoff.emptySelection ||
          kickoff.status?.progress?.emptySelection ||
          kickoff.status?.lastSummary?.emptySelection
        );
        toast(
          kickoff.started
            ? campaignName
              ? `Similarity rescan for campaign "${campaignName}" started. Progress is now live.`
              : sourceKey
            ...
            : kickoff.alreadyRunning
              ? "Similarity backfill is already running."
              : emptySelection
                ? "No maps matched the current scoped similarity selection."
                : "Similarity backfill did not start.",
          "info"
        );
        return;
      } catch (error) {
...
function onInput(e) {
  ...
  if (similaritySourceSelect instanceof HTMLSelectElement) {
    const previousSourceKey = String(state.namingSimilaritySourceKey || "").trim().toLowerCase();
...
async function onSubmit(e) {
  ...
  if (similarityWeightsForm) {
    ...
    const mapUid = String(fd.get("mapUid") || "").trim();
    const scope = String(fd.get("scope") || "map").trim().toLowerCase();
    const action = String(e.submitter?.value || fd.get("weightAction") || "save").trim().toLowerCase();
    if (!mapUid) {
...
async function handleJobAction(action, jobKey) {
  ...
  await guarded(`${action}:${jobKey}`, async () => {
    try {
      ...
      await Promise.all([
        loadDashboard(),
        loadJobs({ source: `job-action:${action}`, forceConsole: true }),
      ]);
    } catch (error) {
      logJobsConsole("job action failed", {
        action,
        jobKey: String(jobKey || "").trim() || null,
        message: error?.message || String(error || "Unknown error."),
      }, "error");
...
async function doLogout() {
  await guarded("logout", async () => {
    ...
    closeDrawer(); state.auth = { authenticated: false, loginUrl: normalizeLoginUrl("/admin/login/") };
...
async function api(url) {
  for (let attempt = 0; attempt <= FETCH_NETWORK_RETRY_ATTEMPTS; attempt += 1) {
    ...
    if (r.status === 401 || r.status === 403) { location.replace(normalizeLoginUrl(p?.loginUrl || state.auth?.loginUrl || "/admin/login/")); throw new Error("Unauthorized"); }
    if (!r.ok) {
