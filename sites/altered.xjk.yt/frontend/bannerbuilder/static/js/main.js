...
const dashmapMessage = pageConfig.dashmapMessage || "Dashmap uploads are not configured on this server.";
...
window.__bannerCapabilities = window.__bannerCapabilities || new Map();
...
function rememberActiveBanner(id, capability) {
  if (!id || !capability) return;
  window.__activeBannerIds.add(id);
  window.__bannerCapabilities.set(id, capability);
}

function getActiveBannerRecords() {
  return getActiveBannerIds()
    .map((id) => ({ id, capability: window.__bannerCapabilities.get(id) || "" }))
    .filter((record) => record.capability);
}

async function refreshActiveBanners() {
  const banners = getActiveBannerRecords();
...
function releaseActiveBanners() {
  ...
  window.__bannerCapabilities.clear();
...
generateBtn.onclick = async () => {
  ...
  try {
    ...
    const { id, url, capability } = await res.json();
    window.__latestBannerId = id;
    window.__selectedBannerId = id;
    rememberActiveBanner(id, capability);
    ensureKeepalive();
...
dashBtn.onclick = async () => {
  ...
  try {
    ...
    const capability = window.__bannerCapabilities.get(id) || "";
    const res = await fetch(`api/banners/${id}/dashmap`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
