...
import { renderAbuseRows } from "./admin-rendering.js?v=2";
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function fetchAbuse(days) {
    renderAbuseRows(abuseBody, []);
    abuseBody.querySelector("td").textContent = "Loading…";
    const res = await fetch(`abuse?days=${days}`);
    const data = res.ok ? await res.json() : [];
    renderAbuseRows(abuseBody, data);
  }
  ...
  masterChk?.addEventListener("change", () => {
    document.querySelectorAll(".card input[type=checkbox]").forEach((c) => (c.checked = masterChk.checked));
    updateCounter();
  });
  ...
  document.addEventListener("click", (e) => {
    ...
    if (e.target.closest(".actions") || e.target.closest("input") || e.target.closest("a")) return;
