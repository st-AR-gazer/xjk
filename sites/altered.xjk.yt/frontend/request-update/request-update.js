...
import { esc, escN, relTime as formatRelativeTime } from "../shared/formatters.js?v=2";
...
function parseMeta(map) {
  ...
  let alteration = "";
  let mapNum = "";
  const campMatch = campaign.match(/^(Spring|Summer|Fall|Winter)\s+(\d{4})(?:\s*[-\u2013]\s*(.+))?$/i);
  if (campMatch) {
...
function populateFilters() {
  ...
  fillSelect(
    $filterMapNum,
    "All",
    [...mapNums].sort((a, b) => Number(a) - Number(b))
  );
}
...
function renderResults() {
  ...
  const hasInput =
    state.query || state.filters.season || state.filters.year || state.filters.alteration || state.filters.mapNum;
...
async function submitForMap(map) {
  ...
  try {
    ...
    showStatus(`Request submitted for "${map.name}". It will be reviewed and processed.`, "success");
...
async function boot() {
  renderHistory();
  await loadMaps();
  if (state.query || state.filters.season || state.filters.year || state.filters.alteration || state.filters.mapNum) {
    renderResults();
  }
}
