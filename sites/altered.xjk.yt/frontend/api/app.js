...
function groupEndpoints(endpoints) {
  ...
  return [...grouped.entries()]
    .map(([group, items]) => ({
      group,
      items: items.sort((a, b) => String(a?.title || a?.path || "").localeCompare(String(b?.title || b?.path || ""))),
    }))
    .sort((a, b) => {
...
async function loadCatalog() {
  state.catalog = await fetchJson(alteredUrl("/api/v1/public/endpoints"));
  renderAll();
}
...
async function handleMapFetch(event) {
  ...
  try {
    const payload = await fetchJson(
      alteredUrl(`/api/v1/public/maps/${encodeURIComponent(mapUid)}?wrHistoryLimit=${wrHistoryLimit}`)
    );
...
boot().catch((error) => {
  ...
  globalThis.XjkSafeHtml.set(
    el.endpointSections,
    `<section class="doc-section"><p class="inline-empty">${esc(error.message || "Failed to load catalog.")}</p></section>`
  );
  el.sidebarGroups.replaceChildren();
