...
function setDropReady(dropEl, file) {
  if (file) dropEl.classList.add("ready");
  else dropEl.classList.remove("ready");
}
...
function renderReplaySummary(replay) {
  ...
  replaySummary.appendChild(renderSummaryItem("Replay Player", replay?.playerNickname || replay?.playerLogin || "Unknown"));
  replaySummary.appendChild(renderSummaryItem("Replay Author", replay?.authorNickname || replay?.authorLogin || "Unknown"));
...
function renderGhostList(ghosts) {
  ...
  ghosts.forEach((ghost) => {
    ...
    details.innerHTML =
      `Login: ${ghost.ghostLogin || "N/A"}<br>` +
      `Zone: ${ghost.ghostZone || "N/A"}<br>` +
      `Trigram/Club: ${(ghost.ghostTrigram || "-") + " / " + (ghost.ghostClubTag || "-")}<br>` +
      `Model: ${ghost?.playerModel?.author || "?"}:${ghost?.playerModel?.id || "?"}<br>` +
      `GameVersion: ${ghost?.recordData?.gameVersion || "N/A"}<br>` +
...
function onMapPick(file) {
  ...
  mapName.textContent = selectedMap ? `Selected: ${selectedMap.name} (${formatKB(selectedMap.size)})` : "No map selected";
...
function onSourcePick(file) {
  ...
  sourceName.textContent = selectedSource
    ? `Selected: ${selectedSource.name} (${formatKB(selectedSource.size)})`
    : "No source selected";
  setDropReady(sourceDrop, selectedSource);
  resetReplayUi();
  stopReplayInspection();
  setError("");
  setStatus("");
...
goBtn.addEventListener("click", () => {
  ...
  xhr.onprogress = (event) => {
    ...
    if (event.lengthComputable) {
      ...
      progressText.textContent = `${formatKB(event.loaded)}`;
    }
  };
  ...
  xhr.onload = () => {
    ...
    if (xhr.status !== 200) {
      ...
      reader.onload = () => {
        const parsed = parseJsonSafe(reader.result || "");
...
bindDropZone(sourceDrop, sourceInput, onSourcePick);
