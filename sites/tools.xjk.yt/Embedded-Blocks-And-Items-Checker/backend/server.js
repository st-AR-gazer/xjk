...
const TOOL_PATH =
  ...
  firstExistingPath([
    path.join(__dirname, "..", "tools", "EmbeddedBlocksAndItemsChecker.exe"),
  ]);
...
const uploadStorage = multer.diskStorage({
  destination: (_req, file, cb) => {
    ...
    if (file.fieldname === "manualOverrides") return cb(null, UPLOAD_MANUAL_DIR);
...
const upload = multer({
  ...
  fileFilter: (_req, file, cb) => {
    ...
    if (file.fieldname === "manualOverrides") {
...
app.post("/api/check", upload.fields([{ name: "map", maxCount: 1 }, { name: "manualOverrides", maxCount: 1 }]), async (req, res) => {
  ...
  const cleanup = async () => {
    ...
    await Promise.all([safeUnlink(mapFile.path), safeUnlink(manualFile?.path), safeUnlink(outputPath)]);
  };
  ...
  try {
    ...
    if (!parsed) {
      ...
      return res.status(500).json({
        error: code === 0 ? "Tool did not return valid JSON output." : `Tool failed with exit code ${code}.`,
        stderr: stderr || null,
      });
    }
