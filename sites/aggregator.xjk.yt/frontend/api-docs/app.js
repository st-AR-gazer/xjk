...
(function () {
  ...
  const TAB_IDS = ["overview", "names", "projects", "events", "clubs", "database", "recipes", "contribute"];
  ...
  async function boot() {
    ...
    setStatus("Ready");
    globalThis.XjkSafeHtml.set(document.getElementById("authBadges"), renderAuthCards(catalog.auth || {}));
    ...
    globalThis.XjkSafeHtml.set(
      ...
      (catalog.currentRole || []).map((item) => `<li>${escapeHtml(item)}</li>`).join("")
    );
    globalThis.XjkSafeHtml.set(
      document.getElementById("guidelines"),
      (catalog.contributionGuidelines || []).map((item) => `<li>${escapeHtml(item)}</li>`).join("")
    );

    globalThis.XjkSafeHtml.set(
      document.getElementById("recipes"),
      (catalog.clientRecipes || [])
        .map(
          (recipe) => `
    ...
    document.getElementById("namesCount").textContent =
      `${(sectionsById.get("identity")?.endpoints || []).length} routes`;
    document.getElementById("projectsCount").textContent =
      `${(sectionsById.get("projects")?.endpoints || []).length} routes`;
    document.getElementById("eventsCount").textContent =
      `${(sectionsById.get("events")?.endpoints || []).length} routes`;
    ...
    document.getElementById("databaseCount").textContent =
      `${(sectionsById.get("database")?.endpoints || []).length} routes`;
    document.getElementById("recipesCount").textContent = `${(catalog.clientRecipes || []).length} recipes`;
    document.getElementById("contributeCount").textContent =
      `${(sectionsById.get("ingest")?.endpoints || []).length} routes`;
