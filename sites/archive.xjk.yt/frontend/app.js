...
function safeArchiveUrl(value) {
  return safeNavigationHref(normalizeText(value), {
...
function normalizeFile(file, i) {
  ...
  const url = safeArchiveUrl(file.url);
...
function normalizeLink(link, i) {
  ...
  const url = safeArchiveUrl(link.url);
  return url ? { label, url } : null;
}
...
function normalizeGame(game, i) {
  ...
  return {
    ...
    image: safeArchiveUrl(game.image),
...
function normalizeArchive(payload) {
  const games = toArray(payload?.games).map(normalizeGame).filter(Boolean);
...
function renderGamePage(game) {
  ...
  globalThis.XjkSafeHtml.set(back, "&larr; Back to archive");
  ...
  buildsSection.appendChild(el("div", { className: "gp-builds-heading", textContent: "ARCHIVED BUILDS" }));
  ...
  if (game.builds.length === 0) {
    ...
    if (hasCategories) {
      ...
      grouped.forEach((builds, category) => {
        buildsSection.appendChild(el("div", { className: "build-category-head", textContent: category }));
...
function createGameCard(game) {
  ...
  if (game.years) {
    info.appendChild(el("span", { className: "game-years", textContent: game.years }));
  }
  ...
  if (game.platforms.length > 3) {
    pills.appendChild(el("span", { className: "pill", textContent: `+${game.platforms.length - 3}` }));
  }
  ...
  const fileCount = game.builds.reduce((sum, build) => sum + build.files.length, 0);
  const metaText =
    buildCount > 0 ? `Builds: ${buildCount}${fileCount > 0 ? ` · Files: ${fileCount}` : ""}` : "No builds archived yet";
  info.appendChild(el("span", { className: "game-meta", textContent: metaText }));

  info.appendChild(el("span", { className: "game-open", textContent: "Open" }));
...
function renderCatalog() {
  ...
  if (totalVisible === 0) {
    elements.catalog.appendChild(el("div", { className: "empty-state", textContent: "No games match your search." }));
  }
}
...
function showGame(game) {
  ...
  document.title = `${game.name} · archive.xjk.yt`;
  renderGamePage(game);
  window.scrollTo(0, 0);
}
...
async function boot() {
  globalThis.XjkSafeHtml.set(elements.catalog, '<div class="empty-state">Loading archive...</div>');
  ...
  elements.search.addEventListener("input", () => {
    ...
    if (!elements.catalogView.hidden) {
      renderCatalog();
