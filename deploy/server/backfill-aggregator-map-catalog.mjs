...
function main() {
  const trackerDbPath = resolveDatabasePath({
    argumentName: "tracker-db",
    environmentName: "XJK_TRACKER_DB",
    fileName: "altered-tracker.sqlite",
  });
  ...
  const projectKey = normalizeProjectKey(readArgument("project-key", "prod-tracker-main"), "prod-tracker-main");
  const projectName = String(readArgument("project-name", "Prod Tracker Main")).trim() || "Prod Tracker Main";
  const sourceLabel = String(readArgument("source", "catalog-sync")).trim() || "catalog-sync";
  ...
  const beforeAggMaps = Number(aggregatorDb.prepare("SELECT COUNT(*) AS count FROM map_registry").get()?.count || 0);
  const beforeProjectMaps = Number(
    aggregatorDb.prepare("SELECT COUNT(*) AS count FROM project_maps WHERE project_key = ?").get(projectKey)?.count || 0
  );
  ...
  const afterAggMaps = Number(verifyDb.prepare("SELECT COUNT(*) AS count FROM map_registry").get()?.count || 0);
  const afterProjectMaps = Number(
    verifyDb.prepare("SELECT COUNT(*) AS count FROM project_maps WHERE project_key = ?").get(projectKey)?.count || 0
  );
  verifyDb.close();
