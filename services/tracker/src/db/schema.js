export const MIGRATIONS = [
  ...
  CREATE INDEX IF NOT EXISTS idx_player_profiles_display_name ON player_profiles(latest_display_name);
  CREATE INDEX IF NOT EXISTS idx_player_profiles_last_resolved ON player_profiles(last_resolved_at DESC);
  `,
  `
  CREATE TABLE IF NOT EXISTS player_name_history (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    account_id TEXT NOT NULL REFERENCES player_profiles(account_id) ON DELETE CASCADE,
    display_name TEXT NOT NULL,
    observed_at TEXT NOT NULL,
    source TEXT NOT NULL DEFAULT 'external-sync',
    created_at TEXT NOT NULL DEFAULT (CURRENT_TIMESTAMP),
    UNIQUE(account_id, display_name)
  );
  CREATE INDEX IF NOT EXISTS idx_player_name_history_account_observed
    ON player_name_history(account_id, observed_at DESC);
  `,
];
