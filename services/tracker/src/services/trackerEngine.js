class TrackerEngine {
  ...
  setConfig({ enabled, tickSeconds, batchSize, maxCheckIntervalSeconds, leaderboardTopN } = {}) {
    ...
    if (maxCheckIntervalSeconds !== undefined && Number.isFinite(Number(maxCheckIntervalSeconds))) {
      this.maxCheckIntervalSeconds = Math.max(0, Number(maxCheckIntervalSeconds) || this.maxCheckIntervalSeconds);
  ...
  getStatus() {
    return {
      ...
      providerReady: typeof this.provider?.isReady === "boolean" ? this.provider.isReady : Boolean(this.provider),
      mode: this.mode,
      leaderboardTopN: this.leaderboardTopN,
  ...
  async runNow({ reason = "manual" } = {}) {
    ...
    try {
      ...
      for (const map of dueMaps) {
        ...
        let changed = false;
        const oldWrTime = Number(map.wrMs || 0);
        const oldHolder = String(map.wrHolder || "");
        const oldHolderAccountId = String(map.wrAccountId || "").trim();
          ...
          if (this.mode === "leaderboard") {
            ...
            if (top) {
              ...
              accountIds = Array.isArray(result.entries)
                ? result.entries.map((entry) => String(entry?.accountId || "").trim()).filter(Boolean)
                : [];
              changed =
                newWrTime > 0 &&
                (oldWrTime <= 0 ||
            ...
            const candidateAccountId =
              String(result.accountId || "").trim() || `acc-${(candidateHolder || "unknown").toLowerCase()}`;
