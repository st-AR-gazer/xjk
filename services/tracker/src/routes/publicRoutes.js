...
import { parseDelimitedTextValues as parseAccountIds } from "../../../shared/valueUtils.js";
...
function createPublicRoutes(service, { realtimeHub = null } = {}) {
  ...
  router.get("/leaderboards/medals", (req, res) => {
    const trackedOnly = req.query.tracked_only === undefined ? true : parseTrackedOnly(req.query.tracked_only);
  ...
  router.get("/wr/latest", (req, res) => {
    ...
    const events = mode === "leaderboard" ? service.getLeaderboardFeed(limit) : service.getWrFeed(limit);
    res.json({
