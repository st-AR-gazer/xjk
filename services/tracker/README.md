...
- API token auth: `TRACKER_ADMIN_TOKEN` (or legacy fallback `ALTERED_ADMIN_TOKEN`)
...
- `TRACKER_AGGREGATOR_BASE_URL` (example: `http://127.0.0.1:3140/api/v1`)
...
- `TRACKER_WR_WEBHOOK_URL` (example: `http://127.0.0.1:3130/api/v1/webhook/wr`)
...
`deploy/local/start-local.ps1` runs:

- `altered-hub` on `3130`
- `tracker-hub` on `3131` (`TRACKER_MODE=wr`)
- `tracker-aggregator` on `3140`
- `tracker-displayname` on `3141`
- `tracker-club` on `3142`
- `tracker-leaderboard` on `3143` (`TRACKER_MODE=leaderboard`)
