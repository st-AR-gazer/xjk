...
import { installTrackerHttpFoundation } from "../shared/trackerHttpRuntime.js";
...
const trackerTrafficServiceName =
  String(TRACKER_INSTANCE_ID || "").trim() || (TRACKER_MODE === "leaderboard" ? "tracker-leaderboard" : "tracker-wr");
...
const trackerSharedFrontendDir = resolveTrackerSharedFrontendDir(FRONTEND_DIR);
...
installTrackerHttpFoundation({
  ...
  morgan,
  jsonLimit: "2mb",
  reportTraffic: reportTrackerTraffic,
});
...
function requireAdminPageAuth(req, res, next) {
  ...
  return res.redirect(302, buildAdminLoginLocation(req.path, { next: safeNextPath }));
}

app.get("/admin/login", (req, res) => {
  const canonicalLocation = buildCanonicalAdminLocation(req.path);
  if (canonicalLocation) return res.redirect(308, canonicalLocation);
  res.sendFile(path.join(trackerSharedFrontendDir, "admin-login.html"));
});

app.get("/admin/logout", (req, res) => {
  adminAuth.logout({ req, res });
  return res.redirect(302, buildAdminLoginLocation(req.path, { logged_out: "1" }));
});
...
app.get(
  ...
  (req, res, next) => {
    const canonicalLocation = buildCanonicalAdminLocation(req.path);
    return canonicalLocation ? res.redirect(308, canonicalLocation) : next();
  },
  requireAdminPageAuth,
  (_req, res) => {
    res.sendFile(path.join(trackerSharedFrontendDir, "admin.html"));
  }
);

app.use("/tracker-shared", express.static(trackerSharedFrontendDir, { index: false, redirect: false }));
app.use(express.static(FRONTEND_DIR));

app.get(["/", "/index.html"], (_req, res) => {
  res.sendFile(path.join(trackerSharedFrontendDir, "index.html"));
});
