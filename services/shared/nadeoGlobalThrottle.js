...
function sanitizeQueueEntry(raw = {}, nowIso = new Date().toISOString()) {
  ...
  if (!id) return null;
  const statusText = String(raw?.status || "pending").trim().toLowerCase();
  const status =
...
function normalizeQueueState(raw = {}, { stateFile = "", minGapMs = 0, nowIso = new Date().toISOString() } = {}) {
  ...
  const entries = entriesRaw
    .map((entry) => sanitizeQueueEntry(entry, nowIso))
    .filter(Boolean);
  return {
    version: 1,
...
async function mutateQueueState(
  ...
  ensureParentDir(queueFilePath);
  return withQueueLock(queueFilePath, { pollMs, staleAfterMs }, async () => {
    const nowIso = new Date().toISOString();
    const existing = readJsonFile(queueFilePath, {});
    let state = normalizeQueueState(existing, { stateFile, minGapMs, nowIso });
    ...
    writeJsonFile(queueFilePath, normalized);
    return normalized;
  });
}
...
function parseQueueSnapshot(stateFile, { minGapMs = 0, maxItems = 120 } = {}) {
  ...
  const rawQueue = readJsonFile(queueFile, {});
  ...
  const waiters = [...normalized.waiters]
    ...
    .slice(-safeMaxItems);
  const pendingWaiters = waiters.filter(
  ...
  const lastRequestAt =
    Number.isFinite(lastRequestAtMs) && lastRequestAtMs > 0
  ...
  return {
    ...
    oldestPendingSeconds:
      Number.isFinite(oldestPendingMs) && oldestPendingMs > 0
        ? Math.max(0, (nowMs - oldestPendingMs) / 1000)
        : null,
    ...
    secondsSinceLastRequest:
      Number.isFinite(lastRequestAtMs) && lastRequestAtMs > 0
        ? Math.max(0, (nowMs - lastRequestAtMs) / 1000)
        : null,
...
async function waitForGlobalNadeoSlot({
  ...
  const safeMaxWaitMs = Math.max(
    safeMinGapMs,
    toSafeInt(maxWaitMs, Math.max(120000, safeMinGapMs * 10))
  );

  ensureParentDir(safeStateFile);
