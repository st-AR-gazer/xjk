...
Within this service, the composition roots expose named domains instead of one flat method catalog:

- `AlteredService`: `catalog`, `maps`, `monitoring`, `players`, `sources`, and `tracker`
- `AlteredRepository`: `activity`, `admin`, `campaigns`, `catalog`, `configuration`, `ingestion`, `leaderboard`, `mapFiles`, `mappers`, `maps`, `monitoring`, and `naming`

Routes select a service domain, and service implementations select a repository domain. Adding a workflow therefore changes its owning domain rather than a generated global facade. Similarity backfill is further divided into target planning, lifecycle state, external-worker management, and the internal runner under `src/services/altered/mapProcessing/similarityBackfill/`.
...
- `AGGREGATOR_BASE_URL`
- `AGGREGATOR_TOKEN`
- `ALTERED_INTERNAL_TOKEN` (must match Aggregator's `DASH_ALTERED_INTERNAL_TOKEN`)
- `ALTERED_WR_WEBHOOK_SECRET` (must match `TRACKER_WR_WEBHOOK_SECRET`)
