...
class TrackerClient {
  ...
  async ensureAdminSession({ forceRefresh = false } = {}) {
    ...
    if (
      !forceRefresh &&
      this.adminSessionCookie &&
      this.adminSessionExpiresAtMs - nowMs > 30000
    ) {
      return true;
    }
    ...
    this.pendingLoginPromise = (async () => {
      const loginUrl = `${trimTrailingSlash(this.adminBaseUrl)}/auth/login`;
      ...
      this.adminSessionExpiresAtMs = Number.isFinite(expiresAt)
        ? expiresAt
    ...
    const perform = async (attempt = 0) => {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          const message =
            payload?.error || payload?.detail || `Tracker request failed (${response.status}).`;
          return {
            ok: false,
  ...
  async getTrackedMaps(limit = 2000) {
    return this.request(
      this.publicBaseUrl,
      `tracked/maps?limit=${Math.max(1, Number(limit) || 2000)}`
    );
  }
  ...
  async getPlayerNames(accountIds = [], { chunkSize = 50 } = {}) {
    ...
    for (let offset = 0; offset < normalizedAccountIds.length; offset += safeChunkSize) {
      ...
      const map = payload.namesByAccountId && typeof payload.namesByAccountId === "object"
