import {
  ALTERATION_REGEX_BEHAVIOR,
  LEGACY_ALTERATION_REGEX_CATALOG,
} from "./legacyAlterationRegexCatalog.js";
    ...
    Object.entries(catalog || {}).forEach(([rawSlug, rawValue]) => {
      ...
      targetSlugs.forEach((targetSlug) => {
        ...
        const seen = new Set(
          bucket.map(
            (entry) =>
              `${String(entry?.parserPattern || "").trim().toLowerCase()}|${String(entry?.legacyPatternId || "").trim().toLowerCase()}|${String(entry?.pattern || "").trim().toLowerCase()}`
          )
        );
