...
function resolveAlterationGroupingMatch(row = {}, snapshot = null) {
  const grouping =
    snapshot && typeof snapshot === "object" ? snapshot : buildAlterationGroupingSnapshot({}, { loaded: false });
  ...
  const compactItemMap = grouping.compactItemMap instanceof Map ? grouping.compactItemMap : new Map();
  const candidates = uniqueBy(
    [normalizeAlterationGroupingKey(row?.slug), normalizeAlterationGroupingKey(row?.name)].filter(Boolean),
    (value) => value
  );
