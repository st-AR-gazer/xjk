import { clampInt, parseOptionalBoolean, toText } from "../../../shared/valueUtils.js";
...
function recordMapCheckFailure(repository, { schedule, runId, map, checkedAt, error }) {
...
class OpsAutomationService {
  constructor({ repository, trackerClient, monitorConfig = {}, logger = console }) {
    this.repository = repository;
    this.trackerClient = trackerClient;
    this.logger = logger;
    this.repository.ensureDefaults();
    this.scheduler = {
      enabled: monitorConfig.enabled === undefined ? true : Boolean(monitorConfig.enabled),
  ...
  async checkSingleMap({ schedule, runId, map, reason }) {
    ...
    if (!trackerResult.ok) {
      ...
      return recordMapCheckFailure(this.repository, { schedule, runId, map, checkedAt, error });
    }
...
export { OpsAutomationService, recordMapCheckFailure };
