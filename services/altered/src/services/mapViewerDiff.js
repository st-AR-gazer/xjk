...
function buildTransformKey(element) {
  ...
  return [position.x, position.y, position.z, rotation.x, rotation.y, rotation.z, extractScale(element)].join("|");
}
...
function pairExactMatches(targetElements, referenceElements) {
  ...
  for (const key of keys) {
    ...
    const referenceBucket = (referenceBuckets.get(key) || [])
      .slice()
      .sort((left, right) => left.instanceId.localeCompare(right.instanceId, "en"));
...
export { MAP_VIEWER_DIFF_SCHEMA_VERSION, buildMapViewerDiffPayload };
