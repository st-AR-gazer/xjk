...
const PARSER_PROJECT_PATH = path.resolve(PARSER_ROOT_DIR, "GbxMapLayoutParser.csproj");
...
async function runParser(requestPath, responsePath, { timeoutMs = 120000 } = {}) {
  ...
  return new Promise((resolve, reject) => {
    const child = spawn(runner.command, runner.args(requestPath, responsePath), {
      windowsHide: true,
    ...
    child.on("close", (code) => {
      clearTimeout(timer);
      if (code !== 0 && code !== 2) {
        reject(new Error(`GBX parser exited with code ${code}. ${toText(stderr) || toText(stdout) || ""}`.trim()));
        return;
      }
