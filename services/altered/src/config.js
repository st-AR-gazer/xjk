...
import { parseBoolean, parseEnvFile } from "../../shared/envUtils.js";
...
function countAlteredCampaignRows(filePath) {
  ...
  try {
    ...
    return Number(db.prepare("SELECT COUNT(*) AS count FROM altered_campaigns").get()?.count || 0);
  } catch {
    return 0;
  } finally {
    try {
...
function resolveAlteredDataDir({ configuredDir, defaultDir, dbFile }) {
  ...
  const resolvedConfiguredDir = configuredDir ? resolvePathFromCwd(configuredDir, resolvedDefaultDir) : "";
...
const DEFAULT_ALTERATION_GROUPS_FILE = path.resolve(__dirname, "..", "alteration-groups.local.json");
...
const { ALTERED_INTERNAL_TOKEN } = resolveAlteredInternalCredentialEnvironment(process.env);
...
const TRACKER_LEADERBOARD_ADMIN_BASE_URL = normalizeBaseUrl(
  process.env.TRACKER_LEADERBOARD_ADMIN_BASE_URL || `${TRACKER_LEADERBOARD_PUBLIC_BASE_URL}/admin`
);
...
const TRACKER_PROXY_TIMEOUT_MS = Math.max(1000, Number(process.env.TRACKER_PROXY_TIMEOUT_MS || 15000));
const TRACKER_DISPLAYNAME_BASE_URL = normalizeBaseUrl(
  process.env.TRACKER_DISPLAYNAME_BASE_URL || (isLocalStack ? "http://127.0.0.1:3141/api" : "http://127.0.0.1:3041/api")
);
const TRACKER_CLUB_BASE_URL = normalizeBaseUrl(
  process.env.TRACKER_CLUB_BASE_URL || (isLocalStack ? "http://127.0.0.1:3142/api" : "http://127.0.0.1:3042/api")
);
const AGGREGATOR_BASE_URL = normalizeBaseUrl(
  process.env.AGGREGATOR_BASE_URL || (isLocalStack ? "http://127.0.0.1:3140/api" : "http://127.0.0.1:3040/api")
);
const AGGREGATOR_TOKEN = String(process.env.AGGREGATOR_TOKEN || process.env.AGGREGATOR_INGEST_TOKEN || "").trim();
const ALTERED_TRACKER_DISPLAYNAME_ENABLED = parseBoolean(process.env.ALTERED_TRACKER_DISPLAYNAME_ENABLED, true);
const ALTERED_TRACKER_DISPLAYNAME_FALLBACK_LOCAL = parseBoolean(
  process.env.ALTERED_TRACKER_DISPLAYNAME_FALLBACK_LOCAL,
  true
);
const ALTERED_TRACKER_CLUB_ENABLED = parseBoolean(process.env.ALTERED_TRACKER_CLUB_ENABLED, true);
const ALTERED_TRACKER_CLUB_FALLBACK_LOCAL = parseBoolean(process.env.ALTERED_TRACKER_CLUB_FALLBACK_LOCAL, true);
const { ALTERED_WR_WEBHOOK_SECRET } = resolveWrWebhookCredentialEnvironment(process.env);
...
const UBI_OAUTH_CALLBACK_PATH = String(process.env.UBI_OAUTH_CALLBACK_PATH || "/auth/ubisoft/callback").trim();
...
const UBI_OAUTH_ALLOWED_USERNAMES = String(process.env.UBI_OAUTH_ALLOWED_USERNAMES || "")
  ...
  .filter(Boolean);
const ALTERED_SESSION_COOKIE_NAME = String(process.env.ALTERED_SESSION_COOKIE_NAME || "altered_admin_session").trim();
const ALTERED_SESSION_TTL_SECONDS = Math.max(300, Number(process.env.ALTERED_SESSION_TTL_SECONDS || 43200));
const ALTERED_OAUTH_STATE_TTL_SECONDS = Math.max(60, Number(process.env.ALTERED_OAUTH_STATE_TTL_SECONDS || 600));
const ALTERED_OAUTH_FALLBACK_LOCAL_ONLY = parseBoolean(process.env.ALTERED_OAUTH_FALLBACK_LOCAL_ONLY, false);
...
const ALTERED_LIVE_MONITOR_INTERVAL_SECONDS = clampInt(process.env.ALTERED_LIVE_MONITOR_INTERVAL_SECONDS || 21600, {
  min: 60,
  max: 86400,
  fallback: 21600,
});
const ALTERED_LIVE_MONITOR_SCHEDULE_MODE = String(process.env.ALTERED_LIVE_MONITOR_SCHEDULE_MODE || "daily")
  .trim()
  .toLowerCase();
const ALTERED_LIVE_MONITOR_DAILY_HOUR_UTC = clampInt(process.env.ALTERED_LIVE_MONITOR_DAILY_HOUR_UTC || 3, {
  min: 0,
  max: 23,
  fallback: 3,
});
const ALTERED_LIVE_MONITOR_DAILY_MINUTE_UTC = clampInt(process.env.ALTERED_LIVE_MONITOR_DAILY_MINUTE_UTC || 0, {
  min: 0,
  max: 59,
  fallback: 0,
});
const ALTERED_LIVE_DISCOVERY_ENABLED = parseBoolean(process.env.ALTERED_LIVE_DISCOVERY_ENABLED, true);
const ALTERED_LIVE_DISCOVERY_INTERVAL_SECONDS = clampInt(process.env.ALTERED_LIVE_DISCOVERY_INTERVAL_SECONDS || 3600, {
  min: 300,
  max: 86400,
  fallback: 3600,
});
const ALTERED_LIVE_DISCOVERY_CAMPAIGN_LIMIT = clampInt(process.env.ALTERED_LIVE_DISCOVERY_CAMPAIGN_LIMIT || 25, {
  min: 1,
  max: 250,
  fallback: 25,
});
...
const ALTERED_LIVE_ACTIVITY_PAGE_SIZE = clampInt(process.env.ALTERED_LIVE_ACTIVITY_PAGE_SIZE || 250, {
  min: 1,
  max: 250,
  fallback: 250,
});
const ALTERED_LIVE_ACTIVITY_ACTIVE_ONLY = parseBoolean(process.env.ALTERED_LIVE_ACTIVITY_ACTIVE_ONLY, false);
const ALTERED_LIVE_FETCH_MAP_DETAILS = parseBoolean(process.env.ALTERED_LIVE_FETCH_MAP_DETAILS, true);
...
const ALTERED_LIVE_USER_AGENT =
  String(process.env.ALTERED_LIVE_USER_AGENT || "").trim() || "altered.xjk.yt/1.0 (+https://xjk.yt/)";
...
const ALTERED_LIVE_MIN_REQUEST_GAP_MS = Math.max(0, Number(process.env.ALTERED_LIVE_MIN_REQUEST_GAP_MS || 5000));
const ALTERED_MAPPER_NAME_TRACKING_ENABLED = parseBoolean(process.env.ALTERED_MAPPER_NAME_TRACKING_ENABLED, true);
...
const ALTERED_MAPPER_NAME_TRACKING_MIN_REQUEST_GAP_MS = Math.max(
  0,
  Number(
    process.env.ALTERED_MAPPER_NAME_TRACKING_MIN_REQUEST_GAP_MS || process.env.ALTERED_LIVE_MIN_REQUEST_GAP_MS || 5000
  )
);
const ALTERED_MAPPER_NAME_TRACKING_USER_AGENT =
  String(process.env.ALTERED_MAPPER_NAME_TRACKING_USER_AGENT || "").trim() || "altered.xjk.yt/1.0 (+https://xjk.yt/)";
const ALTERED_MAPPER_SYNC_SCHEDULER_ENABLED = parseBoolean(process.env.ALTERED_MAPPER_SYNC_SCHEDULER_ENABLED, true);
...
const ALTERED_MAPPER_SYNC_BATCH_SIZE = clampInt(process.env.ALTERED_MAPPER_SYNC_BATCH_SIZE || 50, {
  min: 1,
  max: 50,
  fallback: 50,
});
const ALTERED_MAPPER_SYNC_PRIORITY_BATCH_SIZE = clampInt(process.env.ALTERED_MAPPER_SYNC_PRIORITY_BATCH_SIZE || 25, {
  min: 1,
  max: 50,
  fallback: 25,
});
const ALTERED_MAPPER_SYNC_PRIORITY_TOP_LIMIT = clampInt(process.env.ALTERED_MAPPER_SYNC_PRIORITY_TOP_LIMIT || 250, {
  min: 1,
  max: 2000,
  fallback: 250,
});
...
const ALTERED_OPS_MONITOR_TICK_SECONDS = clampInt(process.env.ALTERED_OPS_MONITOR_TICK_SECONDS || 120, {
  min: 15,
  max: 86400,
  fallback: 120,
});
const ALTERED_OPS_MONITOR_MAX_MAPS_PER_RUN = clampInt(process.env.ALTERED_OPS_MONITOR_MAX_MAPS_PER_RUN || 5000, {
  min: 1,
  max: 25000,
  fallback: 5000,
});
const ALTERED_MAP_COPY_BACKFILL_ENABLED = parseBoolean(process.env.ALTERED_MAP_COPY_BACKFILL_ENABLED, true);
const ALTERED_MAP_COPY_BACKFILL_BATCH_SIZE = clampInt(process.env.ALTERED_MAP_COPY_BACKFILL_BATCH_SIZE || 250, {
  min: 1,
  max: 2000,
  fallback: 250,
});
const ALTERED_MAP_COPY_MAX_CONCURRENT_DOWNLOADS = clampInt(process.env.ALTERED_MAP_COPY_MAX_CONCURRENT_DOWNLOADS || 4, {
  min: 1,
  max: 32,
  fallback: 4,
});
