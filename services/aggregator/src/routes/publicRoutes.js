...
import { parseBoolean as parseBool } from "../../../shared/envUtils.js";
...
function collectQueryValues(rawValues, { splitWhitespace = true } = {}) {
  return [].concat(rawValues || []).flatMap((value) =>
    Array.isArray(value)
      ? value
      : String(value || "")
...
function createPublicRoutes(repository, { trackerControl = {}, catalogFactory = null } = {}) {
  ...
  router.get("/metrics/leaderboards/coverage", async (req, res) => {
    ...
    try {
      const trackedOnly = req.query.tracked_only === undefined ? true : parseBool(req.query.tracked_only);
  ...
  router.post("/display-names/resolve", (req, res) => {
    ...
    const accountIds = collectQueryValues(
      [body.accountId, body.accountIds, body.account_id, body.account_ids, body["accountId[]"], body["account_id[]"]],
      { splitWhitespace: true }
    ).slice(0, limit);
  ...
  router.get("/display-names/by-name", (req, res) => {
    const query = req.query || {};
    const displayNames = collectQueryValues([query.displayName, query.displayNames, query.name, query.names], {
