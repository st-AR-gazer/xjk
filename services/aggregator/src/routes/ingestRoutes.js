...
import { readRequestToken } from "../../../shared/httpAuth.js";
...
function countDisplayNameEntries(payload = {}) {
  ...
  const namesByAccountId =
    ...
    typeof payload.namesByAccountId === "object" &&
    !Array.isArray(payload.namesByAccountId)
...
function createIngestRoutes(
  repository,
  { ingestToken = "", allowInsecureOpen = false, arlOpenplanetAuthSecret = "", openplanetValidateUrl = "" } = {}
) {
  ...
  router.use((req, res, next) => {
    ...
    if (!ingestToken) {
      ...
      return res.status(503).json({ error: "Ingest authentication is not configured." });
    }
    const supplied = readRequestToken(req, { headerNames: ["x-ingest-token", "x-admin-token"] });
    if (supplied !== String(ingestToken).trim()) {
  ...
  router.post("/display-names/arl", async (req, res) => {
    ...
    if (entryCount > ARL_MAX_BATCH_SIZE) {
      return res
        .status(413)
        .json({ error: `ARL display-name batch too large (${entryCount} > ${ARL_MAX_BATCH_SIZE}).` });
    }
